...
def handle_chat_message(history):
    ...
    # Safely get the text from the last message for logging
    try:
        last_text = history[-1].get('parts', [{}])[0].get('text', '')
        print(f"Handling chat history. Last message: '{last_text[:50]}...'")
    except (IndexError, AttributeError) as e:
        print(f"Could not extract last message text for logging: {e}")
    ...
    for message in history:
        ...
        text = ""
        try:
            # Safely get the text from the parts array
            text = message.get('parts', [{}])[0].get('text', '')
        except (IndexError, AttributeError):
            pass # Keep text as empty string if structure is unexpected
